...
biomod_model <- loadRData(paste0(sps_names[7],"/",sps_names[7],".",sps_names[7],".models.out"))
...
sps_response_data_Goce <- function(biomod_model_Goce, sp_dat_Goce){
  ...
  sp_res <- sp_dat_Goce %>% 
    ...
    mutate(expl.name = case_when(expl.name == "Benthic.Temperature.max" ~ "__A)__    Temperature (°C)",
                                 ...
                                 expl.name == "Surface.Chlorophyll.Mean" ~ "__C)__    Chlorophyll a (mg.m-3)"),
...
response_curve_species_mean_Goce <- na.omit(curve_data_Goce) %>% ##I added na.omit to only plot the selected variables
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
  facet_wrap(~expl.name, scales = 'free_x', nrow = 1) +#
...
response_curve_species_mean_Gset <- na.omit(curve_data_Gset) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
# All ZB together now
curve_data_ALL_ZB <- rbind(curve_data_Goce, curve_data_Gset)

response_curve_species_mean_ALL_ZB <- na.omit(curve_data_ALL_ZB) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  #geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
ggsave("figures/EcolInt_ALL_ZB.png", response_curve_species_mean_ALL_ZB, height = 8, width = 12, dpi = 600)
...
response_curve_species_mean_Gtri <- na.omit(curve_data_Gtri) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Mdes <- na.omit(curve_data_Mdes) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Mqua <- na.omit(curve_data_Mqua) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Msco <- na.omit(curve_data_Msco) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
curve_data_ALL_F <- rbind(curve_data_Gtri, curve_data_Mdes, curve_data_Mqua, curve_data_Msco)

response_curve_species_mean_ALL_F <- na.omit(curve_data_ALL_F) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  #geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
  ...
  scale_color_manual(values = colorRampPalette(RColorBrewer::brewer.pal(9, "Oranges"))(11)[4:8]) +
  coord_cartesian(expand = F, ylim = c(0, 1)) +
  theme(legend.position = 'bottom',
        panel.border = element_rect(colour = "black", fill = NA),
        strip.text = ggtext::element_markdown(hjust = 0),
...
ggsave("figures/EcolInter_ALL_F.png", response_curve_species_mean_ALL_F, height = 8, width = 12, dpi = 600)
...
response_curve_species_mean_Acla <- na.omit(curve_data_Acla) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Aesc <- na.omit(curve_data_Aesc) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Lsol <- na.omit(curve_data_Lsol) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Slat <- na.omit(curve_data_Slat) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Zmar <- na.omit(curve_data_Zmar) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_ALL_PB <- na.omit(curve_data_ALL_PB) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  #geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
  ...
  scale_color_manual(values = colorRampPalette(RColorBrewer::brewer.pal(9, "Greens"))(11)[4:9]) +
  coord_cartesian(expand = F, ylim = c(0, 1)) +
  theme(legend.position = 'bottom',
        panel.border = element_rect(colour = "black", fill = NA),
        strip.text = ggtext::element_markdown(hjust = 0),
...
ggsave("figures/EcolInter_ALL_PB.png", response_curve_species_mean_ALL_PB, height = 8, width = 12, dpi = 600)
...
sps_response_data_Cfra <- function(biomod_model_Cfra, sp_dat_Cfra){
  ...
  sp_res <- sp_dat_Cfra %>% 
    ...
    mutate(expl.name = case_when(expl.name == "Benthic.Temperature.mean" ~ "__A)__    Temperature (°C)",
                                 #expl.name == "Surface.Salinity.Mean" ~ "__B)__    Salinity (PSS)",
                                 expl.name == "Surface.Ice.thickness.Mean" ~ "__B)__    Ice thickness (m)",
                                 expl.name == "Surface.Chlorophyll.Mean" ~ "__C)__    Chlorophyll a (mg.m-3)"),
                                 #expl.name == "Surface.Nitrate.Mean" ~ "__E)__    Nitrate (umol.m-3)",
                                 #expl.name == "Surface.Iron.Mean" ~ "__F)__    Iron (umol.m-3)",
                                 #expl.name == "parmean" ~ "__G)__    PAR (Einstein.m-2.day-1)"),
...
response_curve_species_mean_Cfra <- na.omit(curve_data_Cfra) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Dcon <- na.omit(curve_data_Dcon) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Smut <- na.omit(curve_data_Smut) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Upin <- na.omit(curve_data_Upin) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_ALL_PB_NN <- na.omit(curve_data_ALL_PB_NN) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  #geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
  ...
  scale_color_manual(values = colorRampPalette(RColorBrewer::brewer.pal(9, "Greens"))(11)[4:8]) +
  coord_cartesian(expand = F, ylim = c(0, 1)) +
  theme(legend.position = 'bottom',
        panel.border = element_rect(colour = "black", fill = NA),
        strip.text = ggtext::element_markdown(hjust = 0),
...
ggsave("figures/EcolInter_ALL_PB_NN.png", response_curve_species_mean_ALL_PB_NN, height = 8, width = 12, dpi = 600)
...
response_curve_species_mean_Aatl <- na.omit(curve_data_Aatl) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_Aunc <- na.omit(curve_data_Aunc) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
...
response_curve_species_mean_ALL_F_NN <- na.omit(curve_data_ALL_F_NN) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  #geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
  ...
  scale_color_manual(values = colorRampPalette(RColorBrewer::brewer.pal(9, "Oranges"))(10)[4:8]) +
  coord_cartesian(expand = F, ylim = c(0, 1)) +
  theme(legend.position = 'bottom',
        panel.border = element_rect(colour = "black", fill = NA),
        strip.text = ggtext::element_markdown(hjust = 0),
...
ggsave("figures/EcolInter_ALL_F_NN.png", response_curve_species_mean_ALL_F_NN, height = 8, width = 12, dpi = 600)
...
response_curve_species_mean_Gtig <- na.omit(curve_data_Gtig) %>%
  filter(TSS >= 0.7) %>% 
  ggplot(aes(x = expl.val, y = pred.val, colour = species)) +
  #geom_point(alpha = 0.05) +
  geom_smooth(fill = "grey30") +
  ...
  labs(x = NULL, y = 'Probability of occurence', colour = 'Species') + 
  #scale_color_brewer(type = 'qual', palette = 3) +
  scale_color_manual(values = colorRampPalette(RColorBrewer::brewer.pal(9, "Blues"))(9)[6:9]) +
  coord_cartesian(expand = F, ylim = c(0, 1)) +
  theme(legend.position = 'bottom',
        panel.border = element_rect(colour = "black", fill = NA),
        strip.text = ggtext::element_markdown(hjust = 0),
...
ggsave("figures/EcolInt_Gtig_NNZB.png", response_curve_species_mean_Gtig, height = 8, width = 12, dpi = 600)
